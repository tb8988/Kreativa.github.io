...
const CONFIG = {
    API_ENDPOINTS: {
        THREAT_DATA: 'https://api.kreativatech.com/threats',
        CASE_STUDIES: 'https://cms.kreativatech.com/case-studies',
        APPS: 'https://api.kreativatech.com/apps'
    },
    FALLBACK_DATA: {
...
async function initCaseStudies() {
    ...
    try {
        ...
        const response = await fetch(CONFIG.API_ENDPOINTS.CASE_STUDIES);
        const studies = await response.json();
        
        renderCaseStudies(grid, studies);
        initFilters(studies);
    } catch (error) {
        grid.innerHTML = `<div class="api-error">Failed to load case studies. ${error.message}</div>`;
        renderCaseStudies(grid, CONFIG.FALLBACK_DATA.caseStudies);
    }
}

function initFilters(studies) {
    document.querySelectorAll('.filters button').forEach(btn => {
        btn.addEventListener('click', () => {
            const filter = btn.dataset.filter;
            const filtered = filter === 'all' 
                ? studies 
                : studies.filter(s => s.industry === filter);
            renderCaseStudies(document.querySelector('.case-study-grid'), filtered);
        });
    });
}
...
async function initChat() {
    ...
    async function sendMessage() {
        ...
        try {
            ...
            appendMessage("Sorry, I'm having trouble connecting. Please email us.", 'bot');
        } finally {
...
// ======================
// CTA Buttons (Real Conversions)
// ======================
function initCTAs() {
    // Audit button
    document.getElementById('mainCta').addEventListener('click', () => {
        gtag('event', 'conversion', { event_category: 'CTA' });
        Calendly.initPopupWidget({
            url: document.getElementById('mainCta').dataset.calendly
        });
    });
...
// ======================
document.addEventListener('DOMContentLoaded', () => {
    initServices();      // Already functional
    initCaseStudies();   // Now CMS-powered
    initRadarChart();    // Real API data
    initChat();          // AI integration
    initThreatMap();     // Live threat data
    initCTAs();          // Real conversions
    initParticles();     // Existing
    initCardGlowEffects(); // Existing
});